...
import { isWarmupDialogDue, nextWarmupDialogAt, warmupSlotIndex } from '../../src/modules/accounts/warmupSchedule';
...
describe('warmupSchedule', () => {
  ...
  it('marks interval slot due once schedule time has passed', () => {
    const account = {
    ...
    const at = new Date('2026-05-01T12:00:00Z');
    expect(isWarmupDialogDue(account, at)).toBe(true);
    expect(nextWarmupDialogAt(account, at)?.getTime()).toBeLessThanOrEqual(at.getTime());
  });
});
